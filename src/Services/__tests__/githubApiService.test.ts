...
Deno.test("Should throw NOT FOUND", async () => {
  ...
  assertEquals(result instanceof ServiceError, true);
  assertEquals((result as ServiceError).code, 404);
});
...
Deno.test("Should throw RATE LIMIT", async () => {
  ...
  const result = await provider.requestUserRepository("test");
  assertEquals(result instanceof ServiceError, true);
  assertEquals([419, 404].includes((result as ServiceError).code), true);
});
...
Deno.test("Should throw RATE LIMIT Exceed", async () => {
  ...
  const result = await provider.requestUserRepository("test");
  assertEquals(result instanceof ServiceError, true);
  assertEquals((result as ServiceError).code, 419);
});
