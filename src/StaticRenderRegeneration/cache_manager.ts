...
export class CacheManager {
  ...
  get cacheFilePath(): string {
    const tmpEnv = Deno.env.get("TMPDIR") || Deno.env.get("TEMP") ||
      Deno.env.get("TMP");
    const osTmp = Deno.build.os === "windows" ? "C:\\Windows\\Temp" : "/tmp";
    const base = tmpEnv || osTmp;
    if (!existsSync(base)) {
      const local = ".cache";
      try {
        Deno.mkdirSync(local, { recursive: true });
      } catch {}
      return `${local}/${this.cacheFile}`;
    }
    return `${base}/${this.cacheFile}`;
  }
